...
impl TTF {
    pub fn new(path: &str) -> Self {
        let file = get_asset(path).unwrap();
        ...
        // — HEAD —
        let head_offset = tables.get("head").unwrap().1;
        reader.set_position(head_offset as u64);
        let head = _TTF::head::new(&mut reader);
        debug!("{:#?}", head);

        // — MAXP —
        let maxp_offset = tables.get("maxp").unwrap().1;
        reader.set_position(maxp_offset as u64);
        let maxp = _TTF::maxp::new(&mut reader);
        debug!("{:#?}", maxp);

        // — HHEA —
        let hhea_offset = tables.get("hhea").unwrap().1;
        reader.set_position(hhea_offset as u64);
        let hhea = _TTF::hhea::new(&mut reader);
        debug!("{:#?}", hhea);

        // — HMTX —
        // hmtx is variable‐length, so read it in one go from its offset
        let hmtx_offset = tables.get("hmtx").unwrap().1;
        reader.set_position(hmtx_offset as u64);
        let hmtx = _TTF::hmtx::new(&mut reader, hhea, maxp);
        debug!("{:#?}", hmtx);

        let loca = _TTF::loca::new(&mut reader, maxp, head);
        debug!("loca\n{:#?}", loca);

        let glyf = _TTF::glyf::new(&mut reader, loca.clone(), head, tables.clone());
        debug!("glyf\n{:#?}", glyf);

        let cmap_offset = tables.get("cmap").unwrap().1;
        reader.set_position(cmap_offset as u64);
        let cmap = _TTF::cmap::new(&mut reader);
        debug!("{:#?}", cmap);
...
mod _TTF {
    ...
    impl loca {
        pub fn new(reader: &mut BinaryReader, maxp: maxp, head: head) -> Self {
            let mut data = Vec::new();
            for _ in 0..maxp.numGlyphs + 1 {
                data.push(if head.indexToLocFormat == 0 {
    ...
    impl glyf {
        pub fn new(
            ...
            loca: loca,
            head: head,
            tables: HashMap<String, (u32, u32, u32)>,
        ) -> Self {
            let mut data = Vec::new();
            let offset = tables.get("glyf").unwrap().1;
            for i in 0..loca.data.len() - 1 {
                let multiplier = if head.indexToLocFormat == 0 { 2 } else { 1 };
                let locaoffset = loca.data[i] * multiplier;
